...
@
Moreover, the user can study the gene-wise or tissue-wise correlation by using the \texttt{covmat.ts} function. For example, the identity and sphericity test 
<<>>=
