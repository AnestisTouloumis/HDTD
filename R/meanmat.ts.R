...
if(length(N)!=1 | ((N-round(N))!=0) | (N<=1)) 
   stop("'N' must be a positive integer number greater than or equal to 2")
...
if(voi=="columns" & sum(group.sizes)!=p2)
   stop("The number of variables in the prespecified groups is less than the number of column variables") 
if(voi=="rows" & sum(group.sizes)!=p1)
   stop("The number of variables in the prespecified groups is less than the number of row variables") 
