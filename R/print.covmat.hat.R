...
print.covmat.hat <- function(x, ...) {
    ...
    if (!is.null(x$rows.covmat)) {
        ...
        if (!is.null(x$rows.intensity)) 
            cat("Estimated shrinkage intensity =", round(x$rows.intensity, 
                4), "\n")
        cat("Estimated covariance matrix [1:5,1:5] =\n")
        print(round(x$rows.covmat[1:min(5, x$n.rows), 1:min(5, 
            x$n.rows)], 4))
    }
    if (!is.null(x$cols.covmat)) {
        cat("\nCOLUMN VARIABLES\n")
        if (!is.null(x$cols.intensity)) 
            cat("Estimated shrinkage intensity =", round(x$cols.intensity, 
                4), "\n")
        cat("Estimated covariance matrix [1:5,1:5] =\n")
        print(round(x$cols.covmat[1:min(5, x$n.cols), 1:min(5, 
            x$n.cols)], 4))
    }
}
