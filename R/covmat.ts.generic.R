covmat.ts.generic <- function(datamat, N, centered) {
    ...
    if (p1 >= p2) {
        ...
        Y2 <- 2 * Y2
        if (centered == FALSE) {
            xmean <- matrix(rowMeans(matrix(datamat, p1 * p2, 
                N)), p1, p2)
            sum41 <- sum42 <- sum43 <- 0
            ...
            for (i in seq_len(N)) {
                ...
                sum43 <- sum43 + sum(c(helpmat2) * y[id == i, 
                  id != i])
            ...
            sum52 <- 2 * sum52
            sum53 <- 2 * sum53
            sum54 <- (N^2) * sum541 + 2 * (N - 1) * sum43 - ((N - 
                1)^2) * sum42 - sum52
            sum55 <- (N^2) * sum551 - ((N - 1)^2) * sum42 + 2 * 
                (N - 1) * sum43 - sum53
            Y5 <- ((N - 1) * (N^2 - 3 * N + 3) * sum42 + (2 * 
                N - 3) * (Y2 + sum52 + sum53) + 2 * (N - 3) * 
                (Y4 + sum54 + sum55) - 4 * (N^2 - 3 * N + 3) * 
                sum43 - sum51 * (N^3))/3
        }
    } else {
        ...
        for (i in seq_len(N - 1)) Y2 <- sum(y1[id1 == i, id1 > 
            i]^2) + Y2
        Y2 <- 2 * Y2
        if (centered == FALSE) {
            xmean <- matrix(rowMeans(matrix(datamat, p1 * p2, 
            ...
            sum541 <- sum551 <- 0
            for (i in seq_len(N)) {
                Y3 <- sum(c(datamat[, id == i]) * datamat[, id > 
                  i]) + Y3
            ...
            sum54 <- (N^2) * sum541 + 2 * (N - 1) * sum43 - ((N - 
                1)^2) * sum42 - sum52
            sum55 <- (N^2) * sum551 + 2 * (N - 1) * sum43 - ((N - 
                1)^2) * sum42 - sum53
            Y5 <- ((N - 1) * (N^2 - 3 * N + 3) * sum42 + (2 * 
                N - 3) * (Y2 + sum52 + sum53) + 2 * (N - 3) * 
                (Y4 + sum54 + sum55) - 4 * (N^2 - 3 * N + 3) * 
